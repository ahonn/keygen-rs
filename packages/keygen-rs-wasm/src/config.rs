...
impl TryFrom<&KeygenConfig> for keygen_rs::config::KeygenConfig {
    ...
    fn try_from(cfg: &KeygenConfig) -> Result<Self, Self::Error> {
...
pub fn set_config(config: JsValue) -> Result<(), JsError> {
    ...
    let config = keygen_rs::config::KeygenConfig::try_from(&cfg)
        .map_err(|error| JsError::new(&error.to_string()))?;
    keygen_rs::config::set_config(config).map_err(to_js_error)
}
