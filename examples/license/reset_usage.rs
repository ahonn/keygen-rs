...
async fn main() -> Result<(), Error> {
    ...
    // Set up configuration with Admin Token (required for reset operation)
    ...
    // Retrieve and reset the license usage
    ...
    let updated_license = license.reset_usage().await?;

    println!("Usage reset: {}", updated_license.id);
    println!("  Previous Uses: {:?}", license.uses);
    println!("  Current Uses: {:?}", updated_license.uses);
    println!("  Max Uses: {:?}", updated_license.max_uses);

    // Show the reset amount
    if let Some(old_uses) = license.uses {
        if old_uses > 0 {
            println!("  Reset: {} usage count(s) to 0", old_uses);
        }
    }
