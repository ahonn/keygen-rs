use keygen_rs::{
    ...
    product::{Product, UpdateProductRequest, DistributionStrategy, Platform},
    errors::Error,
};
use std::env;
...
async fn main() -> Result<(), Error> {
    ...
    match Product::get(&product_id).await {
        Ok(product) => {
            ...
            println!("  Current Distribution Strategy: {:?}", product.distribution_strategy);
            ...
            let update_request = UpdateProductRequest {
                ...
                platforms: Some(vec![Platform::Windows, Platform::MacOs, Platform::Linux, Platform::Web]),
                permissions: None,
                metadata: None,
            };
            ...
            match product.update(update_request).await {
                Ok(updated_product) => {
                    ...
                    println!("  New Distribution Strategy: {:?}", updated_product.distribution_strategy);
